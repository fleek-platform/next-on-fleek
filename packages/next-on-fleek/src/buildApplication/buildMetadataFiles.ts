...
async function buildNextStaticHeaders(
	...
	const nopContent = `
# === START AUTOGENERATED @fleek-platform/next-on-fleek IMMUTABLE HEADERS ===
${nextStaticPath}/*
${Object.entries(nextStaticHeaders)
	.map(([header, value]) => `  ${header}: ${value}`)
	.join('\n')}
# === END AUTOGENERATED @fleek-platform/next-on-fleek IMMUTABLE HEADERS ===\n`;
...
async function buildRoutes(outputDir: string, opts: BuildMetadataFilesOpts) {
	...
	try {
		await writeFile(
			...
			JSON.stringify({
				version: 1,
				description: `Built with @fleek-platform/next-on-fleek@${nextOnPagesVersion}.`,
